...
\Intro{
	...
	是真的在打广告啊，嗯，做兰大毕业论文LaTex模板时，顺便介绍一下我写的软件：i兰大易班
...
%中文摘要
\ZhAbstract{这是真的在打广告啊，嗯，做兰大毕业论文latex模板时，顺便介绍一下我写的软件。嗯，好像本科生也不用打广告啦，目前两万多人在用，但是研究生没几个人在用啊，很多在写论文的你们马上就是兰州大学的研究生了吧，试一下i兰大易班？

不要仅仅把它当做广告，这里面有很多latex的用法说明}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.9\textwidth]{figures/sublime.png}
...
\begin{table}[H]
    ...
    \toprule
    名称& 加粗 & 倾斜 & 宋体  & 仿宋 & 黑体 & 雅黑\\
    \midrule
    显示 & \textbf{兰朵儿} & \textit{兰朵儿}  & \songti{兰朵儿} & \fangsong{兰朵儿} & \heiti{兰朵儿} & \yahei{兰朵儿} \\
    显示 & \textbf{ldr} & \textit{ldr}  & \songti{ldr} & \fangsong{ldr} & \heiti{ldr} & \yahei{ldr} \\
    \bottomrule
    \end{tabular}
    \label{tbl_font}
\end{table}
...
注意如果系统没有字体，安装以后在这里自己改，比如雅黑字体，mac、linux需要自己安装，然后根据自己安装的名字在LZUThesis.clc里面修改，windows可能会报错，yahei重复，所以我把这两行注释掉了，mac、linux把这两行打开！！！
...
% 宋体，STSongti-SC-Regular：这个名字请根据自己安装的字体名字（mac字体管理，PostScript名称）进行修改
...
\begin{table}[H]
    \centering
    \caption{公式}
    \begin{tabular}{ccccccccc} % 控制表格的格式
    \toprule
    名称& 分数 & 下角标 & 上角标  & 矢量 & 根号 & 希腊字母 & 点乘 & 叉乘\\
    \midrule
    显示 & $\frac{1}{2}$ & $O_2$  & $a^2$ & $\vec{AB}$ & $\sqrt[2]{3}$ & $\theta$ & $\cdot$ & $\times$\\
...
这个，百度学术等网站都可以导出bibtex格式的参考文献，直接放在bib/database.bib文件里、知网需要用其他东西转换，但是我建议用mendeley这个软件管理文献，然后可以导出bibtex格式的，甚至可以直接复制引用，很方便。
